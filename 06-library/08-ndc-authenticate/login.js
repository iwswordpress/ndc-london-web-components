...
class WPLogin extends HTMLElement {
  ...
  afterDelayEmailHandler() {
    if (this.emailField.value.length < 5) {
      this.errEmail.innerHTML =
        "<span class='error'>5 or more characters</span>";
  ...
  passwordHandler() {
    ...
    if (this.passwordField.previousValue != this.passwordField.value) {
      ...
      this.passwordTimer = setTimeout(
        () => this.afterDelayPasswordHandler(),
        delayTime
      );
    }
  ...
  immediatePasswordHandler(val) {
    if (
      !/^([0-9a-zA-Z]+)|([0-9a-zA-Z][0-9a-zA-Z\\s]+[0-9a-zA-Z]+)$/.test(val)
    ) {
      UI.formatError(this.shadowRoot.getElementById('password'));
      this.errPassword.innerHTML =
        "<span class='error'>Invalid characters</span>";
    } else {
      UI.formatSuccess(this.shadowRoot.getElementById('password'));
      this.errPassword.innerHTML =
        "<span class='success'>VALID characters</span>";
    }
    this.activateSend();
  }
  ...
  checkPassword(val) {
    ...
    if (regEx.test(val)) {
      this.errPassword.innerHTML =
        "<span class='success'>Password is valid.</span>";
  ...
  disconnectedCallback() {
    ...
    this.passwordField.removeEventListener(
      'click',
      this.passwordHandlerListener
    );
  }
}
